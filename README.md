...
- **Versioning built in** — every save is a git commit (skipped when nothing
  ...
  be compared with the current document, restored in full, or reinstated one
  change at a time. **Squash Recent Commits** consolidates the current branch
  since its most recent squash or integrated upstream point, with a commit
  message generated by Toki.
