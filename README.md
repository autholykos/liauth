...
- **Versioning built in** — every save is a git commit (skipped when nothing
  ...
  **Reinstate** controls shown directly in the historical text. A version can
  also be restored in full. **Squash Recent Commits** consolidates the current
  branch since its most recent squash or integrated upstream point, with a
  commit message generated by Toki.
...
- **Concurrent-write detection** — the open file is watched; external writes
  ...
  demand.
- **Project folders** — global search groups matches by folder. The navigator
